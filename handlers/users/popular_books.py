...
async def solve_tasks(message: Message):
    books = await parsing.get_books1.get_books(keyboards.default.menu.main_captions[0])
    caption = f'<b>{books[0]["name"]}\n</b>' \
              f'<i>Оценка:</i> {books[0]["rate"]}\n' \
              f'<i>Автор:</i> {books[0]["author"]}\n' \
              f'<i>Жанр:</i> {books[0]["genre"]}\n' \
...
@dp.message_handler(text=keyboards.default.menu.main_captions[1])
async def theory(message: Message):
    await message.answer(f'Выйдет в следующем обновлении')
    #конец